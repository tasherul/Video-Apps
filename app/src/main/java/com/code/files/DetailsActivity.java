...
public class DetailsActivity extends AppCompatActivity implements CastPlayer.SessionAvailabilityListener, ProgramAdapter.OnProgramClickListener, EpisodeAdapter.OnTVSeriesEpisodeItemClickListener,
    ...
    private  long START_TIME_IN_MILIS = 120000;
    ...
    public void initVideoPlayer(String url, Context context, String type) {
        ...
        switch (type) {
            case "hls":
                ...
                Uri u = Uri.parse(durl(url));
                mediaSource = hlsMediaSource(u, context);
                play(mediaSource);
                //player refresh
                //getTvData(categoryType, id);
                //page refresh
    ...
    public static final String ENCRYPTION_KEY = "123456"; // 128 bit key
    public static final String ENCRYPTION_IV = "0987554989aopoio"; // 16 bytes IV
    ...
    private  String  durl(String url){
        ...
        String x = decryptFromHex(AKEY,url);
        return x;
    }
    private  void auto_timer(String Url){
        //long token = System.currentTimeMillis();
        //player refresh
        //getTvData(categoryType, id);
        //page refresh
        //setContentView(R.layout.activity_details);
        ArrayList<String> elephantList = new ArrayList<>(Arrays.asList(Url.split("-")));
        //elephantList.get(0)
        //elephantList.size()
        int max = elephantList.size();
        long end = parseLong(elephantList.get(max-1));
        long start = parseLong(elephantList.get(max-2));
        long sec  = start - end;
        long milisec =sec * 1000;
        START_TIME_IN_MILIS = milisec;
        mTimeLeftInMilis = milisec;

        if(mTimerRunning) {
            //starrtTimer();
            mCountDownTimer = new CountDownTimer(mTimeLeftInMilis, 1000) {
                @Override
                public void onTick(long milisUnilFinished) {
                    mTimeLeftInMilis = milisUnilFinished;
                    //watchStatusTv.setText("start: " + START_TIME_IN_MILIS+" | end:"+mTimeLeftInMilis);
                }
    ...
    private MediaSource hlsMediaSource(Uri uri, Context context) {
        ...
        DataSource.Factory dataSourceFactory = new DefaultDataSourceFactory(context,
                Util.getUserAgent(context, "APITSOFT"), bandwidthMeter);
    ...
    private MediaSource mediaSource(Uri uri, Context context) {
        return new DefaultMediaSourceFactory(
                new DefaultHttpDataSourceFactory("jplayer")).
                createMediaSource(uri);
    }
    ...
    private  static String AKEY="07e67f755b3aa250";
    private  String TK(String token){
    ...
    private void getTvData(final String vtype, final String vId) {
        ...
        long token2 =  token / 2;
        long token3 =  token / 5;
        String Akey_send = token3+"-" + token + token2;
        AKEY = TK(Akey_send);
        Call<SingleDetailsTV> call = api.getSingleDetails(AppConfig.API_KEY, vtype, vId,Akey_send);
